...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['W1'] = torch.normal(0.0, weight_scale,
         size=(input_dim, hidden_dim), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        h, a_cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, s_cache = Linear.forward(h, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dh, dW2, db2 = Linear.backward(d_score, s_cache)
        dX, dW1, db1 = Linear_ReLU.backward(dh, a_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(2, self.num_layers):
          self.params[f'W{i}'] = torch.normal(0.0, weight_scale, size=(hidden_dims[i-2], hidden_dims[i-1]), dtype=dtype, device=device)
          self.params[f'b{i}'] = torch.zeros(hidden_dims[i-1], dtype=dtype, device=device)

        self.params[f'W{self.num_layers}'] = torch.normal(0.0, weight_scale, size=(hidden_dims[-1], num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        if self.use_dropout:
            h, d_cache = Dropout.forward(h, self.dropout_param)
            cache_dropout.append(d_cache)
        for i in range(2, self.num_layers):
          h, a_cache = Linear_ReLU.forward(h, self.params[f'W{i}'], self.params[f'b{i}'])
          cache.append(a_cache)
          if self.use_dropout:
            h, d_cache = Dropout.forward(h, self.dropout_param)
            cache_dropout.append(d_cache)
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    tmp = (1 - config['decay_rate']) * (dw**2) + config['decay_rate'] * config['cache'] 
    next_w = w - ((config['learning_rate'] * dw) / (tmp.sqrt() + config['epsilon']))
    config['cache'] = tmp
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
