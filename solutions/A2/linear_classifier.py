...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    batch_indices = torch.randint(0, num_train, size=(batch_size, ))
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.001, 0.005, 0.008, 1]
    regularization_strengths = [0.00001, 0.0005, 0.001, 0.1, 0.5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        #Prevent overflow
        sigmoid_scores = scores.exp() / torch.sum(scores.exp())
        loss -= sigmoid_scores[y[i]].log()
        for j in range(num_classes):
            dW[:, j] += (sigmoid_scores[j]) * X[i]
            if j == y[i]:
              dW[:, j] -= X[i]
...
def softmax_loss_vectorized(
    ...
    correct_class_score = sigmoid_scores[torch.arange(num_train), y].view(-1,1)
    loss -= torch.sum(correct_class_score.log())
    sigmoid_scores[torch.arange(num_train), y] -= 1
    dW = torch.mm(X.t(), sigmoid_scores)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.0001, 0.001, 0.005, 0.1]
