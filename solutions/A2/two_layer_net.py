...
def nn_forward_backward(
    ...
    sigmoid_scores = scores.exp() / torch.sum(scores.exp(), dim=1).view(-1, 1)
    correct_class_score = sigmoid_scores[torch.arange(N), y].view(-1,1)
    loss = torch.sum(-correct_class_score.log())
    loss /= N
    loss += (torch.sum(W1*W1) + torch.sum(W2*W2)) * reg
...
def nn_get_search_params():
    ...
    regularization_strengths = [0.0001, 0.0002]
    learning_rate_decays = [0.96, 0.95, 0.97]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    lrs, hidden_sizes, reg_strengthes, lr_decays = get_param_set_fn()
    num_feat = data_dict['X_train'].shape[1]
    num_class = max(data_dict['y_train']) + 1
    for lr in lrs:
      for hidden_size in hidden_sizes:
        for reg in reg_strengthes:
          for lr_decay in lr_decays:
            model = TwoLayerNet(num_feat, hidden_size, num_class)
            train_stats = model.train(data_dict['X_train'], data_dict['y_train'],
             data_dict['X_val'], data_dict['y_val'], lr, lr_decay, reg, 1000, 1000)
