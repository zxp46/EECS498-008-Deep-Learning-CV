...
def count_tensor_elements(x: Tensor) -> int:
    ...
    """
    num_elements = x.clone().flatten().shape[0]
    ##########################################################################
    #                      TODO: Implement this function                     #
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    # Replace "pass" statement with your code
    last_row = x[-1, :]
    third_col = x[:, 2].view(x.shape[0],1)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mm = mean.repeat((x.shape[0],1))
    ss= (torch.sum((x - mm)**2, dim=0) / (x.shape[0] - 1))**0.5
    y = (x - mm) / ss
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    m = torch.max(ls) + 1
    z = torch.zeros(ls.shape[0], m)
    tmp = torch.arange(ls.shape[0]).reshape(ls.shape[0],1)
    z[torch.t(tmp), ls] = -ls.shape[0]
    ...
    print(z)
    xx = torch.cat(xs)
    z2 = torch.zeros(ls.shape[0], xx.shape[0], dtype=xx.dtype)
    z2[z,torch.arange(xx.shape[0])] = xx
    print(z2)
    y = torch.sum(z2.float(), dim = 1) / ls
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    # Replace "pass" statement with your code
    xx = x.clone()
    xx, idxs = torch.sort(xx, stable=True)
    xx2 = xx.clone()
    xx2[0], xx2[1:] = xx[-1], xx[:-1]
    idx2 = torch.t(torch.nonzero(xx - xx2)).flatten()
    indices = idxs[idx2]
